...
if( !defined( 'EDD_SLG_VERSION' ) ) {
	define( 'EDD_SLG_VERSION', '2.1.8' ); // Plugin version
}
...
function edd_slg_admin_notices_for_facebook_SDK() {
    ...
    if ( !get_option( 'edd_slg_fb_sdk_notice_disable' ) && current_user_can('administrator') ) {
        echo '<div class="notice notice-warning is-dismissible ">'
                . '<a style="display: inline-flex;position: static;float: right;padding: 0 15px 10px 28px;margin-top: 12px;font-size: 13px;line-height: 1.23076923;text-decoration: none;" href="?edd_slg_fb_sdk_notice_disable=1" class="">Dismiss</a>'
                . '<p style="">' . __( 'We have upgraded Facebook SDK, So you need to update redirect URL in your Facebook app. Please setup redirect URL to your Facebook app as per new settings called "Facebook Valid OAuth Redirect URL".', 'eddslg' ) . '</p>'
            . '</div>';
    }
}
/**
	...
	if( $edd_slg_set_option == '1.3' ) {
            ...
            if( !empty( $edd_slg_enable_login_page ) ) {
                edd_update_option( 'edd_slg_enable_login_options', array(
                                    'edd_login'     => __( 'Check this box to add social login buttons on Easy Digital Downloads login page.', 'eddslg' ),
                                    'edd_checkout'  => __( 'Check this box to add social login buttons on Easy Digital Downloads checkout page.', 'eddslg' ),
                                    'wp_login'      => __( 'Check this box to add social login buttons on default WordPress login page.', 'eddslg' ),
                                ) );
            }
...
if( class_exists( 'Wpweb_Upd_Admin' ) ) { //Check WPWEB Updater is activated
	...
	$WpwebEddSlgUpdateChecker = new WpwebPluginUpdateChecker (
		'http://wpweb.co.in/Updates/EDDSLG/license-info.php',
		__FILE__,
		EDD_SLG_PLUGIN_KEY
	);
