...
class EddSocialLoginPersistentAnonymous {
    ...
    private static function getSessionID($mustCreate = false) {
        ...
        if (isset($_COOKIE['edd_slg_session'])) {
            if (get_site_transient('n_' . $_COOKIE['edd_slg_session']) !== false) {
                self::$verifiedSession = $_COOKIE['edd_slg_session'];

                return self::$verifiedSession;
            }
        }
        if ($mustCreate) {
            self::$verifiedSession = uniqid('edd_slg', true);

            self::setcookie('edd_slg_session', self::$verifiedSession, time() + DAY_IN_SECONDS, apply_filters('edd_slg_session_use_secure_cookie', false));
    ...
    public static function destroy() {
        ...
        if ($sessionID) {
            self::setcookie('edd_slg_session', $sessionID, time() - YEAR_IN_SECONDS, apply_filters('edd_slg_session_use_secure_cookie', false));

            add_action('shutdown', 'EddSocialLoginPersistentAnonymous::destroy_site_transient');
        }
    }
