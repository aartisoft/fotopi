...
<div class="wrap">
	...
	<h2 class="edd-slg-settings-title"><?php _e( 'Social Login', 'eddslg' ); ?></h2>
	...
	<form action="" method="POST">
		...
		<p><?php _e('Drag and drop the below providers to control their display order.', 'eddslg'); ?></p>
		<table class="edd-slg-sortable widefat">
			<thead>
				<tr>
					...
					<?php
						...
						//do_action( 'edd_slg_social_table_header_settings_before' );
					?>-->
				</tr>
			</thead>
			<tbody>
				<?php
					...
					foreach ( $allnetworks as $key => $value ) {
						...
						$is_network_activated = $edd_options['edd_slg_enable_'.$key];//get_option('edd_slg_enable_'.$key);
				?>
						<tr>
							<?php
							...
							?>
							<td class="edd-slg-table-col"><img class='edd-slg-drag-drop-menu' src="<?php echo EDD_SLG_IMG_URL.'/backend/menu.png'; ?>" title="<?php _e('Drag & Drop', 'eddslg'); ?>"></td>
							...
							?>
							<td class="edd-slg-netowrk-<?php echo $key; ?> edd-slg-table-col"><img src="<?php echo EDD_SLG_IMG_URL.'/backend/images/'.$key.'.png';?>" alt="<?php echo $value;?>" /></td>
							<?php
								//do action to add social table data network
								do_action( 'edd_slg_social_table_data_network_after', $key, $value );
							?>
							<td class="edd-slg-table-col reg-count-label"><?php echo $regusers[$key]['count'];?></td>
							<?php
							...
							<td class="edd-slg-table-col">
								<?php
									...
									if(!empty($is_network_activated) && $is_network_activated == '1') {
										echo "<img class='edd-slg-network-activated' src='" . EDD_SLG_IMG_URL.'/backend/check-mark.png' . "' alt='" . $value . "' />";
									} else {
										_e('-', 'eddslg');
									}
								?>
			...
			<tfoot>
				<tr>
					<th width="3%"></th>
					<th class="col-network"><?php _e( 'Network', 'eddslg');?></th>
					<th width="1%" class="edd-slg-social-none"><?php _e('Change Order', 'eddslg'); ?></th>
					<?php
					...
					<?php
						//do action to add social table header settings after
						//do_action( 'edd_slg_social_table_header_settings_after' );
					?>-->
				</tr>
			</tfoot>
		</table>
	...
	<h3><?php _e( 'Social Networks Registeration Pie Graph', 'eddslg' );?></h3>
	<?php
		$colors = array(
						'facebook'		=>	'A200C2',
						'twitter'		=>	'46c0FB',
