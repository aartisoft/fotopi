...
class EDD_Points_Renderer {
	...
	public function edd_points_user_log_list(){
		...
		$tot_points = edd_points_get_user_points( $current_user->ID ); 
			...
			$html .= '	<h4>'.sprintf( __( 'You have %d %s', 'eddpoints' ), $tot_points, $pointslabel ).'</h4>';
			
			$html .= '	<div class="edd-user-points"><table border="1" class="edd-points-details">
								<tr>
				...
				if( !empty( $userpointslogcount ) ) { //check user log in not empty
					...
					foreach ( $userpointslog as $key => $value ){
						...
						$date			= $this->model->edd_points_log_time( strtotime( $value['post_date_gmt'] ) );
	...
	public function edd_points_checkout_message_content( $postid ) {
		...
		if( !empty( $totalpoints_earned ) && $totalpoints_earned == 'user_product' ) {

			echo "<div class='edd-points-product-message'>" . __( 'You won\'t generate any points for buying your own product.', 'eddpoints' ) . "</div>";
		} else {
	...
	public function edd_points_message_content( $postid ) {
		...
		if( $post->post_author == $current_user->ID && edd_points_restrict_seller() ) {
			echo "<div class='edd-points-product-message'>" . __( 'You won\'t generate any points for buying your own product.', 'eddpoints' ) . "</div>";
			return;
		}
	...
	public function edd_points_redeem_point_markup() {
		...
		if ( !empty( $available_discount ) && !empty( $redemptionmessage ) && empty( $gotdiscount ) ) {
			...
			$displaydiscprice	= edd_currency_filter( round( $available_discount, 2 ) );
	...
	public function edd_points_add_custom_user_profile_fields( $user ) {
		...
		?>
			<table class="form-table edd-points-user-profile-balance">
				<tr>
					<th>
						<label for="edd_userpoints"><?php _e('Current points balance ', 'eddpoints'); ?></label>
					</th>
					<td>
	...
	public function edd_points_metabox( $post ) {
		...
		$rating_points 		 = get_post_meta( $post->ID, '_edd_points_review_points', true );
		...
		<div id="edd_simple_point">
			<div id="edd_points_rewads_fields">
				<table>
					...
					<tr>
						<td></td>
						<td><span class="description"><?php printf( __( 'Enter a fixed maximum discount amount which restricts the amount of points that can be redeemed for a discount. For example, if you want to restrict the discount on this product to a maximum of %s5, enter 5. This setting overrides the global and category settings. Use 0 to disable point discounts for this product, and blank to use the global/category defaults.', 'eddpoints' ), edd_currency_filter('') );?></span><br/><br/></td>
					</tr>
					...
					<tr>
						<td valign="top" width="20%">
							<label for="_edd_points_review_points"><?php _e( 'Points earned for Rating:', 'eddpoints' ); ?></label>
						</td>
						<td valign="top">
							<?php
							for ( $star_num = 5; $star_num >= 1; $star_num-- ) {

								$val = isset( $rating_points[$star_num] ) ? $rating_points[$star_num] : '';

								echo '<div class="edd_points_sub_field_item">';
								...
								echo '<input type="text" class="small-text" id="_edd_points_review_points" name="_edd_points_review_points['. $star_num .']" value="' . esc_attr( $val ) . '"/>';
								echo __( ' Point(s)', 'eddpoints' );
					...
					<tr>
						<td></td>
						<td><span class="description"><?php printf( __( 'Enter the number of points earned when a customer add a review on any product. This setting overrides the global settings.', 'eddpoints' ), edd_currency_filter('') );?></span></td>
					</tr>
					<?php endif;
					 ...
					 * @package Easy Digital Downloads - Points and Rewards
 					 * @since 2.0.4
			         */
					do_action( 'edd_points_download_metabox_fields_after', $post );?>
				</table>
