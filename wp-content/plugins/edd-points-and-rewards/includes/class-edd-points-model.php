...
class EDD_Points_Model {
	...
	public function edd_points_get_events( $event = '' ) {
		...
		switch ($event) {
			...
			case 'earned_product_review' :
				$value = sprintf( __( '%s earned by rating a product' , 'eddpoints'), ucfirst( $plurallable ) );
				break;

			case 'refunded_purchase_debited' :
				$value = sprintf( __( 'Earned %s debited towards purchase refund' , 'eddpoints'), ucfirst( $plurallable ) );
				break;

			case 'refunded_purchase_credited' :
				$value = sprintf( __( 'Redeemed %s credited back towards purchase refund' , 'eddpoints'), ucfirst( $plurallable ) );
				break;
				
			case 'refunded_sell' :
				$value = sprintf( __( 'Seller earned %s debited towards purchase refund' , 'eddpoints'), ucfirst( $plurallable ) );
				break;
			
			default:
				break;
		}
		return apply_filters('edd_points_get_events', $value , $event );
	}
	 ...
	 * @since 1.3.7
	 */
	 public function edd_points_settings_section( $sections ) {
		$sections['eddpoints'] = __( 'Points and Rewards', 'eddpoints' );
 	 	return $sections;
	 }
	...
	public function edd_points_settings( $settings ) {
		...
		$edd_points_settings = array(
				...
				array(
					...
					'name'	=> __( 'Maximum Per-Product Discount:', 'eddpoints' ),
				...
				array(
					...
					'name' => __( 'Guest User Cart / Checkout Page Message:', 'eddpoints' ),
				...
				array(
					...
					'name' => __( 'Guest User Cart / Checkout Page Buy Message:', 'eddpoints' ),
				...
				array(
					'id' => 'edd_points_history_guest_messages',
					'name' => __( 'Guest User Points History Message:', 'eddpoints' ),
					'desc' => '<p class="description">'.sprintf(__( 'Displayed points history message for guest users to indicate to login into an account to view points of their account. Customize the message using %s. Limited HTML is allowed. Leave blank to disable.', 'eddpoints' ),'{points_label}').'</p>',
					'type' => 'textarea'
				),
				array(
				...
				array(
					'id'	=> 'edd_points_misc_settings',
					'name'	=> '<strong>' . __( 'Misc Settings', 'eddpoints' ) . '</strong>',
					'desc'	=> __( 'Misc Settings', 'eddpoints' ),
					'type'	=> 'header'
				),
				array(
					'id'		=> 'edd_points_revert_points_refund',
					'name'		=> __( 'Enable points removal for refunded orders', 'eddpoints' ),
					'type'		=> 'checkbox',
					'desc'		=> '<p class="description">'.__( 'Specify whether you want to refund earned and redeemed points when order gets refunded.', 'eddprselmt' ).'</p>'
				),
			);
			
			//If EDD Recurring Payment Plugin active
			if( class_exists( 'EDD_Recurring' ) ) { //Add Checkbox Option field to add point for recurrig payment
				$edd_points_settings[]	=array(
					'id'	=>'edd_points_apply_recurring_payments',
					'name'	=>__('Points earned for recurring payment:','eddpoints'),
					'desc'	=>'<p class="description">'.__( 'Check this box if you wanted to allow users to earn points when recurring payment done.', 'eddpoints' ).'</p>',
					'type'	=>'checkbox',
					'size'	=>'regular'
				);
			}

			//If EDD Reviews Plugin active
			if( class_exists( 'EDD_Reviews' ) ) { //Add settings for recurrig payment

				$edd_reviews_settings	= array(
					array(
						'id'	=> 'edd_points_review_options',
						'name'	=> '<strong>' . __( 'EDD Reviews Settings', 'eddpoints' ) . '</strong>',
						'desc'	=> __( 'Configure EDD Reviews Points Settings', 'eddpoints' ),
						'type'	=> 'header'
					),
					array(
						'id'	=> 'edd_points_enable_reviews',
						'name'	=> __( 'Enable Rating Points:', 'eddpoints' ),
						'desc'	=> '<p class="description">'.__( 'Check this box if you want to assign points to customers when they add a review on any product.', 'eddpoints' ).'</p>',
						'type'	=> 'checkbox',
						'size'	=>'regular'
					),
					array(
						'id'	=> 'edd_points_review_points',
						'name'	=> __( 'Points earned for Rating:', 'eddpoints' ),
						'desc'	=> '<p class="description">'.__( 'Enter the number of points earned when a customer add a review on any product.', 'eddpoints' ).'</p>',
						'type'	=> 'ratingpoints',
						'size'	=> 'small'
					),
				);

				$edd_points_settings = array_merge( $edd_points_settings, $edd_reviews_settings );
			}

			// If EDD is at version 2.5 or later
		    if ( version_compare( EDD_VERSION, 2.5, '>=' ) ) {
		    	$edd_points_settings = array( 'eddpoints' => $edd_points_settings );
		    }
		
		return apply_filters( 'edd_points_settings_after', array_merge( $settings, $edd_points_settings ) );
	}
	 ...
	 * @since 1.0.0
	 **/
	public function edd_points_get_user_checkout_points( $cartdata, $customer_id = null ,$discount = 0 ) {
		...
		if( !empty( $cartdata ) ) {
			...
			foreach ( $cartdata as $key => $item ) {
				...
				if( $post->post_author == $current_user->ID ) {
					...
					$points 			= $this->edd_points_get_earning_points( $item['id'], $itemoptions, true,$customer_id );
					
					//calculate total points for item in cart
					$itemtotalpoints	= !empty( $points ) ? ( $points * edd_get_cart_item_quantity( $item['id'], $itemoptions ) ) : 0;
	 ...
	 * @since 1.0.0
	 **/
	public function edd_points_get_earning_points( $downloadid, $priceoptions = array(), $checkout = false,$customer_id = null ) { 
		
		//if this function called from checkout page then use third parameter to TRUE
		global $edd_options, $current_user;
		
		// get download data
		$download_data = get_post( $downloadid );
		
		// Don't show message if the user is vendor of download except only admin
		if( $download_data->post_author == $customer_id && edd_points_restrict_seller() )
			return;
	...
	public function edd_points_get_max_points_discount_for_download( $downloadid ){
		...
		$max_discount = !empty( $edd_options['edd_points_max_discount'] ) ? $edd_options['edd_points_max_discount'] : '';
	 ...
	 *
	 * Handles to modify cart subtotal if fees 
	 * are applied on product basis
	 * 
	 * @package Easy Digital Downloads - Points and Rewards
	 * @since 1.0.4
	 **/
	public function edd_points_get_cart_subtotal() {

		global $edd_options;

		$cart_subtotal = edd_get_cart_subtotal();
		...
		if ( !empty ( $edd_fees ) ) {
			...
			foreach ( $edd_fees as $edd_fee_key => $edd_fee_val ) {
				...
				if ( strpos ( $edd_fee_key, 'points_redeem_' ) !== false ) {

					$cart_subtotal += abs( $edd_fee_val['amount'] );
				}
			}
		}
		
		return apply_filters( 'edd_points_cart_subtotal', $cart_subtotal );
	}
	...
	public function edd_points_get_discount_for_redeeming_points() {
		...
		//get cart subtotal
		$cartsubtotal = $this->edd_points_get_cart_subtotal() - edd_get_cart_discounted_amount();
		...
		$max_discount = !empty( $edd_options['edd_points_cart_max_discount'] ) ? $edd_options['edd_points_cart_max_discount'] : '';
	...
	public function edd_points_get_payment_discount( $payment_id ) {
		...
		foreach ( $pointsdiscount as $key => $fee ) {
			...
			if ( isset( $fee['id'] ) && strpos ( $fee['id'], 'points_redeem' ) !== false ) {
	...
	/**
	 * Guest User Message for Points History
	 * 
	 * Handles to return guest user points
	 * history message
	 * 
	 ...
	 * @since 1.4.2
	 **/
	public function edd_points_guest_points_history_message( ) {
		
		global $edd_options;
		
		 $message = '';
		
		//check guest user points history message messsage is not empty
		if( !empty( $edd_options['edd_points_history_guest_messages'] ) ) {
			
			//message 
			$guestmessage = $edd_options['edd_points_history_guest_messages'];
			
			//points lable
			$points_label 		= $this->edd_points_get_points_label( 1 );
