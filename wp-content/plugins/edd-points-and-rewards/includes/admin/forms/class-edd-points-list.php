...
class edd_points_log extends WP_List_Table {
	...
	function edd_points_display_points() {
		...
		$all_user_data = $this->model->edd_points_get_points( array( 'edd_points_list' => true ) );
		
		if( !empty( $all_user_data['data'] ) ) {
			
			foreach ( $all_user_data['data'] as $key => $value ) {
				// Get log author
    ...
    function extra_tablenav( $which ) {
    	...
    	if( $which == 'top' ) {
    		...
    		echo '<div class="alignleft actions edd-points-dropdown-wrapper">';
				...
				$all_events = apply_filters( 'edd_points_all_events_options', array(
									'earned_purchase' 				=> __( 'Order Placed', 'eddpoints' ),
									'earned_sell' 					=> __( 'Downloads Sell', 'eddpoints' ),
									'refunded_sell' 				=> __( 'Seller debit for refund order', 'eddpoints' ),
									'redeemed_purchase' 			=> __( 'Order Redeem', 'eddpoints' ),
									'refunded_purchase_debited' 	=> __( 'User debit for Refund Order', 'eddpoints' ),
									'refunded_purchase_credited' 	=> __( 'Credit for Refund Order', 'eddpoints' ),
									'signup' 						=> __( 'Account Signup', 'eddpoints' ),
									'manual' 						=> __( 'Manual', 'eddpoints' ),
									'reset_points' 					=> __( 'Reset Points', 'eddpoints' ),
									'earned_product_review' 		=> __( 'Product Review', 'eddpoints' )
								) );
				$checked = '';?>
