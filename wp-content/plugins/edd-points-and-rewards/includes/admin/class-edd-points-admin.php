...
class EDD_Points_Admin {
	...
	public function edd_points_meta_fields_save( $post_id ) {
		...
		$edd_points_earned = ( !empty($edd_points_earned) ) ? round( edd_sanitize_amount( $edd_points_earned ) ) : $edd_points_earned;
		...
		if( class_exists( 'EDD_Reviews' ) ) { //save product level

			//update Review points
			$review_points = array();
			if( !empty( $_POST['_edd_points_review_points'] ) ) {
				foreach ( $_POST['_edd_points_review_points'] as $key => $value ) {
					$review_points[$key] = trim( $value ) != '' ? edd_sanitize_amount( $value ) : '';
				}
			}
			update_post_meta( $post_id, '_edd_points_review_points', $review_points );
		}
	}
	...
	public function edd_points_settings_validate( $input ) {
		...
		$input['edd_points_earn_conversion']['points'] 	= isset( $input['edd_points_earn_conversion']['points'] ) ? trim( $input['edd_points_earn_conversion']['points'] ) : '';
		...
		$input['edd_points_earn_conversion']['rate'] 	= isset( $input['edd_points_earn_conversion']['rate'] ) ? trim( $input['edd_points_earn_conversion']['rate'] ) : '';
		...
		$input['edd_points_redeem_conversion']['points'] = isset( $input['edd_points_redeem_conversion']['points'] ) ? trim( $input['edd_points_redeem_conversion']['points'] ) : '';
		...
		$input['edd_points_redeem_conversion']['rate'] = isset( $input['edd_points_redeem_conversion']['rate'] ) ? trim( $input['edd_points_redeem_conversion']['rate'] ) : '';
		...
		$input['edd_points_buy_conversion']['points'] = isset( $input['edd_points_buy_conversion']['points'] ) ? trim( $input['edd_points_buy_conversion']['points'] ) : '';
		...
		$input['edd_points_buy_conversion']['rate'] = isset( $input['edd_points_buy_conversion']['rate'] ) ? trim( $input['edd_points_buy_conversion']['rate'] ) : '';
		...
		$input['edd_points_selling_conversion']['points'] = isset( $input['edd_points_selling_conversion']['points'] ) ? trim( $input['edd_points_selling_conversion']['points'] ) : '';
		...
		$input['edd_points_selling_conversion']['rate'] = isset( $input['edd_points_selling_conversion']['rate'] ) ? trim( $input['edd_points_selling_conversion']['rate'] ) : '';
		...
		$edd_points_cart_max_discount = isset( $input['edd_points_cart_max_discount'] ) ? trim( $input['edd_points_cart_max_discount'] ) : '';
		...
		$edd_points_max_discount = isset( $input['edd_points_max_discount'] ) ? trim( $input['edd_points_max_discount'] ) : '';
		$edd_points_max_discount = ( !empty($edd_points_max_discount) ) ? edd_sanitize_amount( $edd_points_max_discount ) : $edd_points_max_discount;
		
		// Sanitizing points earned for account signup
		$edd_points_earned_account_signup = isset( $input['edd_points_earned_account_signup'] ) ? trim( $input['edd_points_earned_account_signup'] ) : '';
		...
		$input['edd_points_history_guest_messages'] 	= $this->model->edd_points_escape_slashes_deep( $input['edd_points_history_guest_messages'], true, true );
			...
			if ( is_array( $payment_ids ) ) {
				...
				foreach( $payment_ids as $payment_id ) {
					...
					if(!empty($userdata['id'])) {
						$checkoutpoints = $this->model->edd_points_get_user_checkout_points( $cartdata , $userdata['id']);
					}
					//check checkout points should not empty which has redeemed by buyer
					if( isset($checkoutpoints) && !empty( $checkoutpoints ) ) {
					...
					// Function to give points to seller
					$this->public->edd_points_add_seller_points_for_complete_purchase($payment_id);
	...
	/**
	 * Delete points from previous orders
	 * 
	 * Handles to Delete points from previous orders
	 *
	 * @package Easy Digital Downloads - Points and Rewards
	 * @since 1.0.0
	 */
	
	public function edd_points_delete_for_previous_orders() {
		
		if( isset( $_GET['points_action'] ) && $_GET['points_action'] == 'delete_points'
			&& isset( $_GET['page'] ) && $_GET['page'] == 'edd-settings' ) {
			...
			$redirectargs = array( 
									...
									'tab'				=>	'extensions', 
									'section'			=>	'eddpoints', 
									'settings-updated' 	=>	'delete_points', 
									'success_count' 	=>	$success_count, 
									'points_action' 	=>	false 
								); 
			$redirect_url = add_query_arg( $redirectargs, admin_url( 'edit.php' ) );
	...
	public function add_hooks() {
		...
		add_filter( 'edd_settings_sections_extensions', array( $this->model, 'edd_points_settings_section' ) );
		
		//add filter to extension settings field
		add_filter( 'edd_settings_extensions_eddpoints_sanitize', array( $this, 'edd_points_settings_validate' ) );
		...
		// add action to delete points from previous orders
		add_action( 'admin_init', array( $this, 'edd_points_delete_for_previous_orders' ) );
