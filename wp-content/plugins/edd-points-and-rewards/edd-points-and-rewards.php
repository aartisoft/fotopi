...
if( !defined( 'EDD_POINTS_VERSION' ) ) {
	define( 'EDD_POINTS_VERSION', '2.1.0' ); // Plugin version
}
...
if ( ! defined( 'EDD_POINTS_PLUGIN_KEY' ) ) {
	define( 'EDD_POINTS_PLUGIN_KEY', 'eddpar' );
}
...
function edd_points_install() {
	...
	//check enable points for edd recurring plugin
	if( !isset( $edd_options['edd_points_apply_recurring_payments'] ) ) {
		$edd_options['edd_points_apply_recurring_payments'] = '';
		$udpopt = true;
	} //end if
	
	if( !isset( $edd_options['edd_points_history_guest_messages'] ) ) {
		$edd_options['edd_points_history_guest_messages'] = sprintf(__( 'Sorry, You have not earned any %s yet.','eddpoints' ),'<strong>{points_label}</strong>');
		$udpopt = true;
	}
...
if( class_exists( 'Wpweb_Upd_Admin' ) ) { //Check WPWEB Updater is activated
	...
	// Plugin updates
	wpweb_queue_update( plugin_basename( __FILE__ ), EDD_POINTS_PLUGIN_KEY );
	...
	$WpwebEddPARUpdateChecker = new WpwebPluginUpdateChecker (
		'http://wpweb.co.in/Updates/EDDPAR/license-info.php',
		__FILE__,
		EDD_POINTS_PLUGIN_KEY
	);
	 ...
	 * @since 1.0.0
	 */
	function edd_par_add_secret_key( $query ) {
		
		$plugin_key	= EDD_POINTS_PLUGIN_KEY;
		
		$query['lickey'] = wpweb_get_plugin_purchase_code( $plugin_key );
		return $query;
	}
	
	$WpwebEddPARUpdateChecker->addQueryArgFilter( 'edd_par_add_secret_key' );
