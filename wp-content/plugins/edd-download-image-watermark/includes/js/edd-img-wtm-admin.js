jQuery(document).ready(function($) {	
	...
	//Media Uploader
	$( document ).on( 'click', '.edd-img-uploader', function() {
	
		var imgfield,showfield;
		imgfield = jQuery(this).prev('input').attr('id');
		showfield = jQuery(this).parents('td').find('.edd-img-wtm-img-view');
		 
		if(typeof wp == "undefined" || EddImgWtm.new_media_ui != '1' ){// check for media uploader
			...
			window.send_to_editor = function(html) {
				...
				if(imgfield)  {
					...
					mediaurl = mediaurl.replace(EddImgWtm.upload_base_url,'');
			...
			file_frame.on( 'insert', function() {
				...
				selection.each( function( attachment, index ) {
					...
					if(index == 0){
						...
						media = attachment_url.replace(EddImgWtm.upload_base_url,'');
						$('#'+imgfield).val(media);
...
function edd_img_wtm_regenerate_plugin_info(){
